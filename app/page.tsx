  ...
  return (
    <div className="min-h-screen w-full bg-gray-100 flex flex-col items-center p-8">
      <input
        ...
        className="mb-6 p-3 w-full max-w-lg rounded-full border border-gray-300 shadow-lg"
      />

      <div className="flex justify-center gap-4 mb-6">
        <button
          className={`px-6 py-3 rounded-full text-lg ${filter === "all" ? "bg-yellow-600 text-white" : "bg-gray-300"} hover:bg-yellow-700 transition duration-300`}
          onClick={() => setFilter("all")}
        >
          Todos
        </button>
        <button
          className={`px-6 py-3 rounded-full text-lg ${filter === "hero" ? "bg-blue-600 text-white" : "bg-gray-300"} hover:bg-blue-700 transition duration-300`}
          onClick={() => setFilter("hero")}
        >
          Superhéroe
        </button>
        <button
          className={`px-6 py-3 rounded-full text-lg ${filter === "villain" ? "bg-red-600 text-white" : "bg-gray-300"} hover:bg-red-700 transition duration-300`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-6xl">
        {filteredCharacters.map((char, index) => (
          <div
            key={index}
            className="p-6 bg-white rounded-3xl shadow-lg flex flex-col items-center hover:shadow-xl transition duration-300"
          >
